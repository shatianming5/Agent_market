...
def _write_json_ref(path: Path, payload: dict) -> dict:
...
def _write_doctor_checkpoint(
    root: Path,
    cfg: StrategyLoopConfig,
    rows: list[dict],
    *,
    pareto_pool: dict | None = None,
    final_status: dict | None = None,
    final_promotion: dict | None = None,
    state_overrides: dict | None = None,
    top_overrides: dict | None = None,
) -> dict:
    promotion = dict(final_promotion or {"promoted": False})
    status = dict(final_status or {"selected": None, "finalists": [], "promotion": promotion, "promoted": False})
    pool = dict(pareto_pool or {"finalists": []})
    scored = [
        row
        for row in rows
    ...
    candidate_paths = [
        ...
        for row in rows
        if isinstance(row, dict) and str(row.get("candidate_path") or "").strip()
    ]
    state = {
        "run_id": cfg.run_id,
    ...
    payload.update(top_overrides or {})
    _write_json(root / "checkpoint.json", payload)
    return payload
...
def test_strategy_loop_doctor_accepts_complete_formal_run(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    pareto_pool = {"axes": {axis: [pareto_axis_row]}, "finalists": [pareto_finalist]}
    _write_json(root / "leaderboard.json", {"rows": [leaderboard_row]})
    ...
    selected_candidate = {
        **candidate,
        ...
        "metadata": {},
        "strategy_path": None,
        "path": str(root / "iter_01" / "candidate.json"),
    ...
    _write_json(
        ...
        {"artifact_refs": {"evaluation.json": evaluation_ref, "verification.json": verification_ref}},
    )
    _write_json(
        root / "checkpoint.json",
        {
            ...
            "state": {
                ...
                "iteration": 2,
                "phase": "PREPARE",
                "best_candidate": leaderboard_row,
                "best_score": leaderboard_row["score"],
                "score_history": [leaderboard_row],
                "candidate_paths": [source_candidate_path],
                "status": LOOP_COMPLETED,
    ...
    assert result["summary"]["selected_evaluation_bindings_checked"] == 1
    assert result["summary"]["selected_evaluation_binding_mismatches"] == 0
    assert result["summary"]["selected_finalist_bindings_checked"] == 1
    assert result["summary"]["selected_finalist_binding_mismatches"] == 0
    assert result["summary"]["selected_finalist_best_rank_mismatches"] == 0
    assert result["summary"]["selected_verification_payload_bindings_checked"] == 1
    ...
    assert result["summary"]["checkpoint_score_history_binding_mismatches"] == 0
    assert result["summary"]["checkpoint_best_candidate_bindings_checked"] == 1
    assert result["summary"]["checkpoint_best_candidate_binding_mismatches"] == 0
    assert result["summary"]["checkpoint_candidate_path_bindings_checked"] == 1
    assert result["summary"]["checkpoint_candidate_path_binding_mismatches"] == 0
    assert result["summary"]["checkpoint_pareto_pool_bindings_checked"] == 1
    assert result["summary"]["checkpoint_pareto_pool_binding_mismatches"] == 0
    assert result["summary"]["checkpoint_final_status_bindings_checked"] == 1
    ...
    assert result["summary"]["pareto_finalist_leaderboard_binding_mismatches"] == 0
    assert result["summary"]["pareto_finalist_axis_bindings_checked"] == 1
    assert result["summary"]["pareto_finalist_axis_binding_mismatches"] == 0
...
def test_strategy_loop_doctor_binds_checkpoint_score_history_to_leaderboard(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_checkpoint_score_history_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    _write_doctor_checkpoint(
        ...
        final_promotion=promotion,
        state_overrides={"score_history": [{**row, "score": 2.0}]},
    )
    ...
    assert result["ok"] is False
    assert result["summary"]["checkpoint_score_history_bindings_checked"] == 1
    assert result["summary"]["checkpoint_score_history_binding_mismatches"] == 1
    assert result["summary"]["checkpoint_best_candidate_binding_mismatches"] == 0
    assert result["summary"]["checkpoint_candidate_path_binding_mismatches"] == 0
    assert "checkpoint score_history differs from leaderboard rows" in messages


def test_strategy_loop_doctor_binds_checkpoint_best_and_candidate_paths(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_checkpoint_best_candidate_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    rows: list[dict] = []
    for iteration, top_k, score in ((1, 1, 1.0), (2, 2, 3.0)):
        idir = root / f"iter_{iteration:02d}"
        candidate = {"candidate_type": "rank_profile", "rank_profile": {"top_k": top_k}}
        candidate_ref = _write_json_ref(idir / "candidate.json", candidate)
        evaluation = {
            "iteration": iteration,
            "candidate": candidate,
            "candidate_path": f"artifacts/factor_strategy_loop/{run_id}/iter_{iteration:02d}/candidate.json",
            "score": score,
            "score_components": {"composite_score": score},
            "constraints_ok": True,
            "metrics": {},
            "research_metrics": {},
            "freqtrade_metrics": {},
            ...
            "promotion_eligible": False,
            "pareto_eligible": True,
            "parameter_signature": f"unit-signature-{iteration}",
            "artifact_refs": {"candidate.json": candidate_ref},
            "promotion": {"promoted": False},
        }
        _write_json(idir / "evaluation.json", evaluation)
        evaluation_ref = strategy_loop_mod._artifact_ref(idir / "evaluation.json")
        _write_json(idir / "manifest.json", {"artifact_refs": {"candidate.json": candidate_ref, "evaluation.json": evaluation_ref}})
        rows.append(strategy_loop_mod._leaderboard_row_from_evaluation(evaluation, run_id, iteration=iteration))
    promotion = {"promoted": False}
    final_status = {"selected": None, "finalists": [], "promotion": promotion, "promoted": False}
    pareto_pool = {"finalists": []}
    _write_json(root / "manifest.json", {"cli_args": cfg.__dict__, "git": strategy_loop_mod._git_provenance()})
    _write_json(root / "leaderboard.json", {"rows": rows})
    _write_json(root / "pareto_pool.json", pareto_pool)
    _write_json(root / "final_blind_status.json", final_status)
    _write_json(root / "final_promotion.json", promotion)
    _write_doctor_checkpoint(
        ...
        state_overrides={
            ...
            "best_score": 99.0,
            "candidate_paths": ["artifacts/factor_strategy_loop/other_run/iter_01/candidate.json"],
        },
    )
    ...
    assert result["summary"]["checkpoint_score_history_binding_mismatches"] == 0
    assert result["summary"]["checkpoint_best_candidate_bindings_checked"] == 1
    assert result["summary"]["checkpoint_best_candidate_binding_mismatches"] == 2
    assert result["summary"]["checkpoint_candidate_path_bindings_checked"] == 1
    assert result["summary"]["checkpoint_candidate_path_binding_mismatches"] == 1
    assert "checkpoint best_candidate differs from highest-scoring leaderboard row" in messages
    assert "checkpoint best_score differs from highest-scoring leaderboard row" in messages
    assert "checkpoint candidate_paths differ from leaderboard candidate paths" in messages


def test_strategy_loop_doctor_binds_checkpoint_final_artifacts(
    tmp_path: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_checkpoint_final_artifact_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    final_status = {"selected": None, "finalists": [], "promotion": promotion, "promoted": False}
    pareto_pool = {"finalists": []}
    stale_status = {"selected": None, "finalists": [], "promotion": {"promoted": True}, "promoted": True}
    _write_json(root / "manifest.json", {"cli_args": cfg.__dict__, "git": strategy_loop_mod._git_provenance()})
    _write_json(root / "leaderboard.json", {"rows": [row]})
    _write_json(root / "pareto_pool.json", pareto_pool)
    _write_json(root / "final_blind_status.json", final_status)
    ...
    _write_doctor_checkpoint(
        ...
        final_promotion=promotion,
        state_overrides={
            "final_blind_status": stale_status,
            "final_promotion": {"promoted": True},
        },
        top_overrides={"final_blind_status": stale_status},
    )
    ...
    assert result["ok"] is False
    assert result["summary"]["checkpoint_final_status_bindings_checked"] == 1
    assert result["summary"]["checkpoint_final_status_binding_mismatches"] == 2
    assert result["summary"]["checkpoint_final_promotion_bindings_checked"] == 1
    assert result["summary"]["checkpoint_final_promotion_binding_mismatches"] == 1
...
def test_strategy_loop_doctor_binds_pareto_pool_rows_to_leaderboard(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_pareto_leaderboard_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    axis_row = strategy_loop_mod._compact_leaderboard_row(leaderboard_row)
    axis_row["axis_value"] = strategy_loop_mod._axis_value(axis, leaderboard_row)
    tampered_axis_row = {**axis_row, "score": 2.0}
    _write_json(root / "checkpoint.json", {"config": cfg.__dict__, "state": {"run_id": run_id, "iteration": 1}})
    _write_json(root / "manifest.json", {"cli_args": cfg.__dict__, "git": strategy_loop_mod._git_provenance()})
    _write_json(root / "leaderboard.json", {"rows": [leaderboard_row]})
    _write_json(root / "pareto_pool.json", {"axes": {axis: [tampered_axis_row]}, "finalists": [tampered_axis_row]})
    _write_json(root / "final_blind_status.json", {"selected": None, "finalists": [], "promotion": {"promoted": False}})
    _write_json(root / "final_promotion.json", {"promoted": False})
    ...
    assert result["ok"] is False
    assert result["summary"]["pareto_axis_leaderboard_bindings_checked"] == 1
    assert result["summary"]["pareto_axis_leaderboard_binding_mismatches"] == 1
    assert result["summary"]["pareto_finalist_leaderboard_bindings_checked"] == 1
    assert result["summary"]["pareto_finalist_leaderboard_binding_mismatches"] == 1
    assert "pareto_pool axis row differs from leaderboard compact row" in messages
    assert "pareto_pool finalist differs from leaderboard compact row" in messages


def test_strategy_loop_doctor_binds_pareto_finalist_axes_to_axis_rows(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_pareto_axis_membership_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    source_ref = _write_json_ref(root / "iter_01" / "candidate.json", candidate)
    axis = "best_validation_composite"
    wrong_axis = "best_research_robustness"
    leaderboard_row = {
        "iteration": 1,
        "candidate": candidate,
        "candidate_path": f"artifacts/factor_strategy_loop/{run_id}/iter_01/candidate.json",
    ...
    _write_json(root / "manifest.json", {"cli_args": cfg.__dict__, "git": strategy_loop_mod._git_provenance()})
    _write_json(root / "leaderboard.json", {"rows": [leaderboard_row]})
    _write_json(root / "pareto_pool.json", {"axes": {axis: [axis_row]}, "finalists": [axis_row]})
    _write_json(root / "final_blind_status.json", {"selected": None, "finalists": [], "promotion": {"promoted": False}})
    _write_json(root / "final_promotion.json", {"promoted": False})
    ...
    assert result["summary"]["pareto_finalist_leaderboard_binding_mismatches"] == 0
    assert result["summary"]["pareto_finalist_axis_bindings_checked"] == 1
    assert result["summary"]["pareto_finalist_axis_binding_mismatches"] == 1
    assert "pareto_pool finalist axes differ from axis membership" in messages


def test_strategy_loop_doctor_flags_source_artifact_hash_mismatch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_source_hash_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    _write_json(
        root / "final_blind_status.json",
        {
            "selected": {
                ...
                "verification_status": VERIFICATION_PASSED,
                "lean_gate": {"status": VERIFICATION_PASSED, "comparison_status": "ok"},
                "artifact_refs": {"evaluation.json": bad_ref},
            },
            "promotion": {"promoted": False},
            "deepresearch": {
                "artifacts": {
...
def test_strategy_loop_doctor_rejects_deepresearch_artifacts_outside_run(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_deepresearch_outside_run"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    outside_root = repo_paths.artifacts_root() / "strategy_deepresearch" / "other_run"
    _write_json(outside_root / "context.json", {"run_id": "other_run"})
    deepresearch["artifacts"]["context"] = "artifacts/strategy_deepresearch/other_run/context.json"
    blind_dir = root / "blind_1"
    evaluation_ref = _write_json_ref(blind_dir / "evaluation.json", {"status": "selected"})
    verification_ref = _write_json_ref(blind_dir / "verification.json", {"status": VERIFICATION_PASSED})
    _write_json(blind_dir / "lean_gate.json", {"status": VERIFICATION_PASSED, "comparison_status": "ok"})
...
def test_strategy_loop_doctor_rejects_deepresearch_final_status_payload_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_deepresearch_final_status_drift"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    _write_json(blind_dir / "manifest.json", {"artifact_refs": {"evaluation.json": evaluation_ref, "verification.json": verification_ref}})
    final_status = {"promoted": False, "selected": selected, "promotion": promotion}
    stale_context_status = {**final_status, "selected": {**selected, "score": 9.0}}
    final_status["deepresearch"] = _write_deepresearch_artifacts(run_id, final_status=stale_context_status)
    _write_json(root / "final_blind_status.json", final_status)
    ...
    assert result["summary"]["deepresearch_context_final_binding_mismatches"] == 1
    assert "deepresearch context final_status differs from final_blind_status" in messages
...
def test_strategy_loop_doctor_binds_selected_to_blind_evaluation_artifact(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_selected_blind_eval_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    mismatched_evaluation = {**selected, "score": 5.0}
    _write_json(blind_dir / "evaluation.json", mismatched_evaluation)
    evaluation_ref = strategy_loop_mod._artifact_ref(blind_dir / "evaluation.json")
    _write_json(blind_dir / "lean_gate.json", {"status": VERIFICATION_PASSED, "comparison_status": "ok"})
    _write_json(blind_dir / "manifest.json", {"artifact_refs": {"evaluation.json": evaluation_ref, "verification.json": verification_ref}})
    final_status = {"promoted": False, "selected": selected, "promotion": {"promoted": False}}
    final_status["deepresearch"] = _write_deepresearch_artifacts(run_id, final_status=final_status)
    _write_json(root / "final_blind_status.json", final_status)
    ...
    assert result["ok"] is False
    assert result["summary"]["selected_evaluation_bindings_checked"] == 1
    assert result["summary"]["selected_evaluation_binding_mismatches"] == 1
    assert "selected blind finalist differs from blind evaluation artifact" in messages


def test_strategy_loop_doctor_binds_selected_candidate_to_candidate_artifact(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_selected_candidate_payload_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    blind_dir = root / "blind_1"
    candidate = {"candidate_type": "rank_profile", "rank_profile": {"top_k": 1}}
    selected_candidate = {"candidate_type": "rank_profile", "rank_profile": {"top_k": 2}}
    candidate_ref = _write_json_ref(blind_dir / "candidate.json", candidate)
    verification_ref = _write_json_ref(blind_dir / "verification.json", {"status": VERIFICATION_PASSED})
    selected = {
        "score": 10.0,
    ...
    assert result["summary"]["selected_evaluation_bindings_checked"] == 1
    assert result["summary"]["selected_evaluation_binding_mismatches"] == 0
    assert result["summary"]["selected_candidate_payload_bindings_checked"] == 1
    assert result["summary"]["selected_candidate_payload_binding_mismatches"] == 1
    assert "selected blind finalist candidate differs from candidate artifact" in messages


def test_strategy_loop_doctor_binds_selected_verification_and_lean_payloads(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_selected_gate_payload_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    candidate = {"candidate_type": "rank_profile", "rank_profile": {"top_k": 1}}
    candidate_ref = _write_json_ref(blind_dir / "candidate.json", candidate)
    verification_ref = _write_json_ref(blind_dir / "verification.json", {"status": VERIFICATION_PASSED, "source": "artifact"})
    selected = {
        "score": 10.0,
        "blind_final": True,
        "promotion_eligible": True,
        "verification_status": VERIFICATION_PASSED,
        "verification": {"status": VERIFICATION_PASSED, "source": "embedded"},
        "lean_gate": {"status": VERIFICATION_PASSED, "comparison_status": "ok", "source": "embedded"},
        "candidate": candidate,
        "candidate_path": f"artifacts/factor_strategy_loop/{run_id}/blind_1/candidate.json",
        "parameter_signature": "unit-signature",
        "artifact_refs": {"candidate.json": candidate_ref},
    ...
    _write_json(blind_dir / "evaluation.json", selected)
    evaluation_ref = strategy_loop_mod._artifact_ref(blind_dir / "evaluation.json")
    _write_json(blind_dir / "lean_gate.json", {"status": VERIFICATION_PASSED, "comparison_status": "ok", "source": "artifact"})
    _write_json(blind_dir / "manifest.json", {"artifact_refs": {"evaluation.json": evaluation_ref, "verification.json": verification_ref}})
    final_status = {
        "promoted": False,
        "selected": selected,
    ...
    assert result["ok"] is False
    assert result["summary"]["selected_verification_payload_bindings_checked"] == 1
    assert result["summary"]["selected_verification_payload_binding_mismatches"] == 1
    assert result["summary"]["selected_lean_gate_payload_bindings_checked"] == 1
    assert result["summary"]["selected_lean_gate_payload_binding_mismatches"] == 1
    assert "selected blind finalist verification differs from verification artifact" in messages
    assert "selected blind finalist LEAN gate differs from LEAN gate artifact" in messages


def test_strategy_loop_doctor_requires_promoted_selected_gate_payloads(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_promoted_missing_selected_gate_payloads"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag=run_id,
    ...
    promotion = {
        ...
        "artifact_refs": {"optimized_profile": strategy_loop_mod._artifact_ref(optimized_profile_path)},
    ...
    assert result["ok"] is False
    assert result["summary"]["selected_verification_payload_bindings_checked"] == 0
    assert result["summary"]["selected_verification_payload_binding_mismatches"] == 1
    assert result["summary"]["selected_lean_gate_payload_bindings_checked"] == 0
    assert result["summary"]["selected_lean_gate_payload_binding_mismatches"] == 1
...
def test_strategy_loop_doctor_requires_selected_to_be_represented_in_finalists(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_selected_missing_from_finalists"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    _write_json(blind_dir / "lean_gate.json", {"status": VERIFICATION_PASSED, "comparison_status": "ok"})
    _write_json(blind_dir / "manifest.json", {"artifact_refs": {"evaluation.json": evaluation_ref, "verification.json": verification_ref}})
    other_eval = {**selected, "score": 9.0}
    final_status = {
        "promoted": False,
        "selected": selected,
        "finalists": [{"evaluation": other_eval, "score": other_eval["score"], "promotion_eligible": True, "blind_final": True}],
        "promotion": {"promoted": False},
    }
    final_status["deepresearch"] = _write_deepresearch_artifacts(run_id, final_status=final_status)
    _write_json(root / "final_blind_status.json", final_status)
    ...
    assert result["ok"] is False
    assert result["summary"]["selected_finalist_bindings_checked"] == 1
    assert result["summary"]["selected_finalist_binding_mismatches"] == 1
    assert "selected blind finalist is not represented in final_blind_status finalists" in messages


def test_strategy_loop_doctor_requires_selected_to_be_top_scoring_eligible_finalist(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_selected_not_top_finalist"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    _write_json(blind_dir / "lean_gate.json", {"status": VERIFICATION_PASSED, "comparison_status": "ok"})
    _write_json(blind_dir / "manifest.json", {"artifact_refs": {"evaluation.json": evaluation_ref, "verification.json": verification_ref}})
    higher_eval = {**selected, "score": 20.0, "parameter_signature": "higher-signature"}
    final_status = {
        "promoted": False,
        "selected": selected,
        "finalists": [
            {"evaluation": selected, "score": selected["score"], "promotion_eligible": True, "blind_final": True},
            {"evaluation": higher_eval, "score": higher_eval["score"], "promotion_eligible": True, "blind_final": True},
        ],
        "promotion": {"promoted": False},
    }
    final_status["deepresearch"] = _write_deepresearch_artifacts(run_id, final_status=final_status)
    ...
    assert result["summary"]["selected_finalist_bindings_checked"] == 1
    assert result["summary"]["selected_finalist_binding_mismatches"] == 0
    assert result["summary"]["selected_finalist_best_rank_mismatches"] == 1
    assert "selected blind finalist is not highest-scoring promotion eligible finalist" in messages
...
def test_strategy_loop_doctor_flags_final_promotion_mismatch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_promotion_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag="unit",
    ...
    _write_json(root / "leaderboard.json", {"rows": [{"iteration": 1, "promotion_eligible": False}]})
    _write_json(root / "pareto_pool.json", {"finalists": []})
    _write_json(root / "final_promotion.json", {"promoted": False, "artifacts": {}, "reason": "root reason"})
    deep_root = repo_paths.artifacts_root() / "strategy_deepresearch" / run_id
    _write_json(deep_root / "context.json", {"run_id": run_id})
    _write_json(deep_root / "sources.json", {"sources": []})
    _write_json(
        root / "final_blind_status.json",
        {
            ...
            "promotion": {"promoted": False, "artifacts": {}, "reason": "status reason"},
            "deepresearch": {
                "artifacts": {
                    "context": f"artifacts/strategy_deepresearch/{run_id}/context.json",
                    "sources": f"artifacts/strategy_deepresearch/{run_id}/sources.json",
    ...
    assert result["ok"] is False
    assert any(item["message"] == "final_promotion.json differs from final_blind_status promotion" for item in result["findings"])


def test_strategy_loop_doctor_requires_promoted_optimized_profile_artifact(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_missing_promoted_artifact"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag=run_id,
    ...
    assert result["ok"] is False
    assert result["summary"]["promoted_artifact_files"] == 0
    assert any(item["message"] == "promoted artifact path does not exist: optimized_profile" for item in result["findings"])


def test_strategy_loop_doctor_rejects_promoted_artifact_hash_mismatch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_promoted_artifact_hash_mismatch"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag=run_id,
    ...
    _write_json(optimized_profile_path, optimized_profile)
    promotion = {
        "promoted": True,
        "artifacts": {"optimized_profile": f"artifacts/rank_portfolio/{run_id}/optimized_profile.json"},
        "artifact_refs": {"optimized_profile": strategy_loop_mod._artifact_ref(optimized_profile_path)},
    ...
    optimized_profile_path.write_text(json.dumps(optimized_profile, indent=2), encoding="utf-8")
    _write_json(root / "checkpoint.json", {"config": cfg.__dict__, "state": {"run_id": run_id, "iteration": 1}})
    _write_json(root / "manifest.json", {"cli_args": cfg.__dict__, "git": strategy_loop_mod._git_provenance()})
    _write_json(root / "leaderboard.json", {"rows": [{"iteration": 1, "promotion_eligible": False}]})
    _write_json(root / "pareto_pool.json", {"finalists": []})
    ...
    assert result["ok"] is False
    assert result["summary"]["promoted_artifact_refs_checked"] == 1
    assert result["summary"]["promoted_artifact_refs_hash_mismatch"] == 1
    assert "promotion artifact refs failed integrity check" in messages
...
def test_strategy_loop_doctor_rejects_optimized_profile_eval_payload_drift(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("AGENT_MARKET_ARTIFACTS_ROOT", str(tmp_path / "artifacts"))
    run_id = "doctor_formal_optimized_eval_payload_drift"
    root = repo_paths.artifacts_root() / "factor_strategy_loop" / run_id
    root.mkdir(parents=True)
    cfg = StrategyLoopConfig.from_args(
        tag=run_id,
    ...
    _write_json(root / "pareto_pool.json", {"finalists": []})
    _write_json(root / "final_promotion.json", promotion)
    final_status = {"promoted": True, "selected": selected, "promotion": promotion}
    final_status["deepresearch"] = _write_deepresearch_artifacts(run_id, final_status=final_status)
    _write_json(root / "final_blind_status.json", final_status)
    ...
    assert result["ok"] is False
    assert result["summary"]["optimized_profile_eval_bindings_checked"] == 1
    assert result["summary"]["optimized_profile_eval_binding_mismatches"] == 1
    assert "optimized_profile evaluation differs from selected blind finalist" in messages
...
def test_promotion_requires_passing_constraints(tmp_path: Path) -> None:
    ...
    assert passed["artifact_refs"]["optimized_profile"]["path"].endswith("/optimized_profile.json")
    assert passed["artifact_refs"]["optimized_profile"]["sha256"]
