...
def _format_design_spec(spec: Any) -> str:
...
def build_prompt(
    ...
    prompt = textwrap.dedent(
        ...
        Design directives:
        {_format_design_spec(design_spec) if design_spec else "None provided"}
...
def _extract_json_object(payload: str) -> Dict[str, Any]:
    try:
        ...
        try:
            ...
            repaired = _repair_trailing_json(match)
            if repaired is not None:
                return repaired
            raise


def _repair_trailing_json(text: str) -> Optional[Dict[str, Any]]:
    """Attempt to salvage responses where the last JSON element is truncated."""
    search = text.rfind("},")
    while search != -1:
        candidate = text[: search + 1] + "\n    ]\n}"
        try:
            return json.loads(candidate)
        except json.JSONDecodeError:
            search = text.rfind("},", 0, search)
    return None
