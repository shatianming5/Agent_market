...
from agent_market.paths import DATA_DIR, PROJECT_ROOT
...
class FeatureDatasetBuilder:
    def __init__(self, config: Dict[str, Any]):
        ...
        if not self.feature_file.is_absolute():
            self.feature_file = (PROJECT_ROOT / self.feature_file).resolve()
    ...
    def build(self) -> Dataset:
        ...
        context = self._build_feature_context(feature_cfg)
    ...
    def _load_pair_dataframe(self, pair: str) -> pd.DataFrame:
        ...
        if not file_path.exists():
            fallback = self.data_dir / f"{sanitized}-{self.timeframe}.feather"
            if fallback.exists():
                file_path = fallback
            else:
    ...
    def _build_feature_context(self, feature_cfg: Dict[str, Any]) -> SimpleNamespace:
        params = dict(feature_cfg.get('feature_parameters') or {})
        corr_list = feature_cfg.get('include_corr_pairlist')
        if corr_list is not None:
            params.setdefault('include_corr_pairlist', corr_list)
        lstm_dir = feature_cfg.get('lstm_prediction_dir')
        if lstm_dir:
            params.setdefault('lstm_prediction_dir', str(lstm_dir))
        raw_config = {'freqai': {'feature_parameters': params}}
        exchange_dir = self.data_dir / self.exchange
        resolved_dir = exchange_dir if exchange_dir.exists() else self.data_dir
