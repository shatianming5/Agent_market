...
class FreqAISettings:
    ...
    def from_file(
        ...
        if not datadir_root.is_absolute():
            candidates = [
                ...
                (path.parent.parent.parent / datadir_root).resolve(),
                (Path.cwd() / datadir_root).resolve(),
                (Path.cwd().parent / datadir_root).resolve(),
        ...
        if not data_dir.exists() and datadir_root.exists():
            data_dir = datadir_root
    ...
    def validate_dataset(self, timeframe: Optional[str] = None) -> Path:
        ...
        if missing:
            ...
            raise FileNotFoundError("Missing historical data files. Please download them first:\n" + formatted)
