...
from agent_market.freqai.analysis import build_feature_dataframe
from agent_market.freqai.context import FeatureContext, resolve_feature_context
...
def test_freqtrade_scripts_import(monkeypatch):
    project_root = Path(__file__).resolve().parents[1]
    freqtrade_root = project_root / "freqtrade"
    monkeypatch.syspath_prepend(str(freqtrade_root))
    module = importlib.import_module("freqtrade.scripts.freqai_expression_agent")
    assert module is not None


def test_build_feature_dataframe_merges_corr_pairs(tmp_path):
    data_root = tmp_path / "data"
    exchange_dir = data_root / "binanceus"
    exchange_dir.mkdir(parents=True)

    dates = pd.date_range("2025-01-01", periods=16, freq="h")
    base_close = np.linspace(100, 104, len(dates))
    base = pd.DataFrame(
        {
            "date": dates,
            "open": base_close - 0.2,
            ...
            "low": base_close - 0.5,
            "close": base_close,
            "volume": np.linspace(1000, 1200, len(dates)),
        }
    )
    corr_close = base_close * 1.01
    corr = pd.DataFrame(
        {
            "date": dates,
            "open": corr_close - 0.3,
            ...
            "low": corr_close - 0.4,
            "close": corr_close,
            "volume": np.linspace(800, 900, len(dates)),
        }
    )

    base_path = exchange_dir / "BTC_USDT-1h.feather"
    corr_path = exchange_dir / "ETH_USDT-1h.feather"
    base.to_feather(base_path)
    corr.to_feather(corr_path)

    feature_cfg = {
        "features": [],
        "include_corr_pairlist": ["ETH/USDT"],
        "feature_combos": [
            {"name": "arb_price_spread_eth", "formula": "close - close_ETH_USDT_1h"},
            {"name": "arb_price_ratio_eth", "formula": "close / (close_ETH_USDT_1h + 1e-9) - 1"},
        ],
    }
    ...
    context = FeatureContext(feature_file=feature_path, timeframe="1h", settings=settings)

    merged = build_feature_dataframe(
        base.copy(),
        feature_cfg,
        context=context,
    ...
    assert "close_ETH_USDT_1h" in merged.columns
    assert "arb_price_spread_eth" in merged.columns
    assert "arb_price_ratio_eth" in merged.columns
    assert "close_BTC_USDT_1h" not in merged.columns

    spread = merged["close"] - merged["close_ETH_USDT_1h"]
    ratio = merged["close"] / (merged["close_ETH_USDT_1h"] + 1e-9) - 1
    assert np.allclose(merged["arb_price_spread_eth"].dropna(), spread.dropna())
    assert np.allclose(merged["arb_price_ratio_eth"].dropna(), ratio.dropna())
...
def test_compute_information_coefficient_basic():
    from agent_market.freqai.analysis import compute_information_coefficient

    data = pd.DataFrame(
        {
            "feat_a": np.linspace(1.0, 10.0, 10),
            "feat_b": np.linspace(10.0, 1.0, 10),
            "close": np.linspace(10.0, 19.0, 10),
        }
    )
    forward_returns = data["close"].pct_change().shift(-1)
    ics = compute_information_coefficient(data, ["feat_a", "feat_b"], forward_returns)
    ic_map = {ic.name: ic.ic_pearson for ic in ics}
    assert ic_map["feat_a"] < 0
    assert ic_map["feat_b"] > 0
